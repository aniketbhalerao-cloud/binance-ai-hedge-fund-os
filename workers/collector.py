"""Worker collector.

:class:`DefaultCollector` gathers the standardized source readings in a
context, normalizes them into :class:`~workers.models.JobSource` entries
(highest-priority-first, capped by ``max_items``), and builds a raw
:class:`~workers.models.JobBatch` with one unplanned entry per source. Queue
typing and dispatch routing are the Planner and Dispatcher stages' job. It is
deterministic and stateless, and it only *collects* — it never modifies any
subject.
"""

from __future__ import annotations

from workers.context import WorkerContext
from workers.exceptions import CollectionError
from workers.models import JobBatch, JobEntry, JobSource

__all__ = ["DefaultCollector"]


class DefaultCollector:
    """Stateless source collection and batch construction."""

    def collect(self, context: WorkerContext) -> JobBatch:
        """Return the raw :class:`JobBatch` for ``context``.

        Raises:
            CollectionError: If an unexpected failure occurs.
        """
        try:
            sources = self._sources(context)
            entries = tuple(JobEntry(source=s) for s in sources)
            return JobBatch(sources=sources, entries=entries)
        except CollectionError:
            raise
        except Exception as exc:  # translate; never leak internals
            raise CollectionError(str(exc)) from exc

    @staticmethod
    def _sources(
        context: WorkerContext,
    ) -> tuple[JobSource, ...]:
        candidates = [
            *context.storage_sources,
            *context.reporting_sources,
            *context.notification_sources,
            *context.monitoring_sources,
            *context.scheduler_sources,
        ]
        # Highest-priority-first, with a stable tiebreak by name for determinism.
        candidates.sort(key=lambda s: (-s.priority, s.source, s.name))
        return tuple(candidates[: context.parameters.max_items])

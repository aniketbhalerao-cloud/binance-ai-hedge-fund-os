...
:class:`InMemorySchedulerRegistry` is a thread-safe store that **owns the
running scheduler records**, keyed by id. It never creates records (creation
...
from scheduler.exceptions import RegistryError
from scheduler.models import SchedulerRecord

__all__ = ["InMemorySchedulerRegistry"]


class InMemorySchedulerRegistry:
    """A thread-safe registry that owns scheduler records, keyed by id."""

    def __init__(self) -> None:
        self._records: dict[str, SchedulerRecord] = {}
        self._lock = Lock()
    ...
    def get(self, record_id: str) -> SchedulerRecord:
        ...
        if record is None:
            raise RegistryError(f"scheduler record {record_id!r} not found")
        return record

...
:class:`InMemoryMonitoringRegistry` is a thread-safe store that **owns the running
monitoring records**, keyed by id. It never creates records (creation is the
...
from monitoring.exceptions import RegistryError
from monitoring.models import MonitoringRecord

__all__ = ["InMemoryMonitoringRegistry"]


class InMemoryMonitoringRegistry:
    """A thread-safe registry that owns monitoring records, keyed by id."""

    def __init__(self) -> None:
        self._records: dict[str, MonitoringRecord] = {}
        self._lock = Lock()
    ...
    def get(self, record_id: str) -> MonitoringRecord:
        ...
        if record is None:
            raise RegistryError(f"monitoring record {record_id!r} not found")
        return record

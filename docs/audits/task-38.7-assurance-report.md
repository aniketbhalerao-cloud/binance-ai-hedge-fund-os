...
## 12. Post-evidence addendum, 2026-08-23 — M-8 (implicit protocol-dispatch discovery gap)

This addendum is appended after this report's original evidence was recorded. **It changes no counter, no hash, and no commit reference above** — `docs/audits/task-38.7-result.json` (hash `bdbe9cffd1b770140af150e2d66a6524e078f17a817f99bb5d1ec771a7731c0b`, `nodes_unresolved=19`, `calls_unresolved=1032`) is unmodified and remains bound to Phase A commit `dcb1ebc6b7fd34e89979fd9adf8ed0d57bc8ac58`, exactly as §§4–5 above describe it.

A follow-up diagnostic pass, conducted after this report's original evidence was recorded, established that those counters cover only **explicit call resolution**: `StaticWalker` discovers and classifies literal `ast.Call` nodes present in a walked callable's own source. It has no corresponding discovery mechanism for execution the CPython interpreter dispatches implicitly, with no `ast.Call` node anywhere in source — iteration (`__iter__`/`__next__`), context managers (`__enter__`/`__exit__`), comparisons, truth testing (including `__len__`-fallback), subscription, membership, hashing, string formatting, arithmetic operators, and property/descriptor access (a bare attribute read, not even a `Call` node in any form). This was reproduced directly, repeatedly, and without ever executing the reproduction: a small set of non-executed, git-untracked scratch fixtures, each embedding a call to a real forbidden identity (`builtins.open`) inside the relevant dunder method, triggered only through the ordinary syntax that invokes that protocol. In every family tested, the resulting call records contained no entry at all for the embedded call — not `unresolved`, simply absent, with no effect on `calls_unresolved`, `nodes_unresolved`, or any other reported field.

This finding is recorded as **`docs/audits/task-38.5-risk-register.md`'s M-8** (Medium, Open) — see that entry for the full evidence description, the complementary role of the harness's own runtime-denial layer (which observes only paths actually executed during a given run, not paths that exist but are never triggered), and the explicit statement that no currently reachable forbidden production path is claimed. Source-controlled regression coverage for this gap, and any future narrowing of it, is left to whichever future task takes up its remediation — not added to `tests/audit_harness/` by this addendum.

**M-8 is entirely outside this report's original counters' accounting** — `nodes_unresolved`/`calls_unresolved` measure explicit-call-resolution completeness only, and always have; M-8 does not change what those numbers mean or what they were already known not to cover (§4's own "not a completeness proof" framing already anticipated a gap of this general kind, without previously naming this specific mechanism).

**Gate outcome: unchanged, HOLD.** This addendum does not itself change any Layer 1 count, does not close H-1 or H-2, and does not move `ADR-032` toward ALLOWED. It does not authorize or reject `builtins.str.join`'s pending ADR-032 Phase 0 review (still paused, unrecorded) and does not modify `EXACT_IDENTITY_POLICY` in any way — M-8 is a discovery-mechanism finding about the walker itself, unrelated to any pending policy-entry decision. See `docs/architecture/decisions/ADR-032-structural-audit-gate.md`'s own Task 38.7 re-evaluation for M-8's citation at the gate level.

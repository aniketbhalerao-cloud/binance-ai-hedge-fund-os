...
| Recommendation | Remediation is out of scope for this finding's own recording and is left to a separately scoped future task. No specific mechanism is selected, prescribed, or implied here — in particular, this finding does not recommend or authorize any form of name/protocol allowlist (an "assume this specific dunder method is safe" table), which would repeat exactly the kind of blanket, non-individually-reviewed trust `audit_harness.identity`'s own design already forbids. Source-controlled regression coverage proving this gap (and any future narrowing of it) belongs in `tests/audit_harness/`, added by whichever future task takes up remediation — not recorded here. |
| Disposition | Open |

...
**Why `app/wiring.py` is the sanctioned composition-root import site:** it is the one place in the codebase permitted to import across every completed framework's `register_<framework>` function and, where dry-run resolution requires it, a framework's concrete implementation, in order to build and validate the whole-system object graph — no other module, in `app/` or elsewhere, gains that permission. Framework-to-framework imports of public domain/value types and `Protocol` interfaces exist elsewhere in the codebase and are intentional (`docs/audits/task-38.5-risk-register.md` H-1; investigated in Task 38.9B, disposition still Open pending its own Phase B/C). No framework directly constructs another framework's concrete implementation — construction of a concrete cross-framework collaborator is DI-container-mediated, and every framework's own `register_<framework>` function (not only `app/wiring.py`) is where that mediated construction is registered. Direct lifecycle/business execution on anything is outside Task 38's dry-run composition scope entirely (see the constraints below), regardless of which module holds a reference.

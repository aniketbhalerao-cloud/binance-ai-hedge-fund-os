...
:class:`InMemoryNotificationRegistry` is a thread-safe store that **owns the running
notification records**, keyed by id. It never creates records (creation is the
...
from notification.exceptions import RegistryError
from notification.models import NotificationRecord

__all__ = ["InMemoryNotificationRegistry"]


class InMemoryNotificationRegistry:
    """A thread-safe registry that owns notification records, keyed by id."""
    ...
    def get(self, record_id: str) -> NotificationRecord:
        ...
        if record is None:
            raise RegistryError(f"notification record {record_id!r} not found")
        return record
